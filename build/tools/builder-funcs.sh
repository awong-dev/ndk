...
builder_begin ()
{
    ...
    if [ -n "$_BUILD_MK" ]; then
        ...
        echo "builddir = $_BUILD_DIR" >> $_BUILD_MK
        echo "rule do" >> $_BUILD_MK
        echo "  command = exit 1" >> $_BUILD_MK
    fi
...
builder_static_library ()
{
    ...
    if [ "$_BUILD_MK" ]; then
        ...
        echo "build $lib: do $_BUILD_OBJECTS" >> $_BUILD_MK
    fi
    if [ -z "${_BUILD_AR}" ]; then
        _BUILD_AR=${AR:-ar}
    fi
    builder_log "${_BUILD_PREFIX}Archive: $libname"
    builder_command rm -f "$lib" "&&" ${_BUILD_AR} crsD "$lib" "$_BUILD_OBJECTS"
...
builder_host_executable ()
{
    ...
    if [ "$_BUILD_MK" ]; then
        ...
        echo "build $exe: do $_BUILD_OBJECTS" >> $_BUILD_MK
    fi
...
{
    if [ "$_BUILD_MK" ]; then
        echo "default $_BUILD_TARGETS" >> $_BUILD_MK
        # HIDE is used to hide the Makefile output, unless --verbose --verbose
        # is used.
        if [ "$VERBOSE2" = "yes" ]; then
            run ninja -v -f $_BUILD_MK
        else
            run ninja -f $_BUILD_MK
        fi
...
builder_begin_android ()
{
    ...
    if [ -z "$LLVM_VERSION" ]; then
        ...
        GCC_BINPREFIX=$NDK_DIR/$(get_toolchain_binprefix_for_arch $ARCH $GCC_VERSION)
        GCC_TOOLCHAIN=$(dirname $(dirname $GCC_BINPREFIX))
    fi
    ...
    _BUILD_LDFLAGS_BEGIN_SO="-nostdlib $CRTBEGIN_SO_O"
    _BUILD_LDFLAGS_BEGIN_EXE="-nostdlib $CRTBEGIN_EXE_O"
