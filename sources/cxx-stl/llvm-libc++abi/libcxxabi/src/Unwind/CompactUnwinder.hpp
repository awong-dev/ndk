...
#warning TODO(danakj): Update this comment with truth for ARM/NEON.
...
int CompactUnwinder_arm<A>::stepWithCompactEncoding(
    compact_unwind_encoding_t compactEncoding, uint64_t functionStart,
    A &addressSpace, Registers_arm &registers) {
#warning TODO(danakj): Is this right for ARM/NEON?
  switch (compactEncoding & UNWIND_ARM_MODE_MASK) {
  case UNWIND_ARM_MODE_FRAME:
    return stepWithCompactEncodingFrame(compactEncoding, functionStart,
                                        addressSpace, registers);
  case UNWIND_ARM_MODE_FRAMELESS:
    return stepWithCompactEncodingFrameless(compactEncoding, functionStart,
                                            addressSpace, registers);
  }
  _LIBUNWIND_ABORT("invalid compact unwind encoding");
...
template <typename A>
int CompactUnwinder_arm<A>::stepWithCompactEncodingFrameless(
    compact_unwind_encoding_t encoding, uint64_t, A &addressSpace,
    Registers_arm &registers) {
#warning TODO(danakj): Is this right for ARM/NEON?
  uint32_t stackSize =
      16 * EXTRACT_BITS(encoding, UNWIND_ARM_FRAMELESS_STACK_SIZE_MASK);

  uint64_t savedRegisterLoc = registers.getSP() + stackSize;

  if (encoding & UNWIND_ARM_FRAME_X19_X20_PAIR) {
    registers.setRegister(UNW_ARM_X19, addressSpace.get64(savedRegisterLoc));
    savedRegisterLoc -= 8;
    registers.setRegister(UNW_ARM_X20, addressSpace.get64(savedRegisterLoc));
    savedRegisterLoc -= 8;
  }
  if (encoding & UNWIND_ARM_FRAME_X21_X22_PAIR) {
    registers.setRegister(UNW_ARM_X21, addressSpace.get64(savedRegisterLoc));
    savedRegisterLoc -= 8;
    registers.setRegister(UNW_ARM_X22, addressSpace.get64(savedRegisterLoc));
    savedRegisterLoc -= 8;
  }
  if (encoding & UNWIND_ARM_FRAME_X23_X24_PAIR) {
    registers.setRegister(UNW_ARM_X23, addressSpace.get64(savedRegisterLoc));
    savedRegisterLoc -= 8;
    registers.setRegister(UNW_ARM_X24, addressSpace.get64(savedRegisterLoc));
    savedRegisterLoc -= 8;
  }
  if (encoding & UNWIND_ARM_FRAME_X25_X26_PAIR) {
    registers.setRegister(UNW_ARM_X25, addressSpace.get64(savedRegisterLoc));
    savedRegisterLoc -= 8;
    registers.setRegister(UNW_ARM_X26, addressSpace.get64(savedRegisterLoc));
    savedRegisterLoc -= 8;
  }
  if (encoding & UNWIND_ARM_FRAME_X27_X28_PAIR) {
    registers.setRegister(UNW_ARM_X27, addressSpace.get64(savedRegisterLoc));
    savedRegisterLoc -= 8;
    registers.setRegister(UNW_ARM_X28, addressSpace.get64(savedRegisterLoc));
    savedRegisterLoc -= 8;
  }

  if (encoding & UNWIND_ARM_FRAME_D8_D9_PAIR) {
    registers.setFloatRegister(UNW_ARM_D8,
                               addressSpace.getDouble(savedRegisterLoc));
    savedRegisterLoc -= 8;
    registers.setFloatRegister(UNW_ARM_D9,
                               addressSpace.getDouble(savedRegisterLoc));
    savedRegisterLoc -= 8;
  }
  if (encoding & UNWIND_ARM_FRAME_D10_D11_PAIR) {
    registers.setFloatRegister(UNW_ARM_D10,
                               addressSpace.getDouble(savedRegisterLoc));
    savedRegisterLoc -= 8;
    registers.setFloatRegister(UNW_ARM_D11,
                               addressSpace.getDouble(savedRegisterLoc));
    savedRegisterLoc -= 8;
  }
  if (encoding & UNWIND_ARM_FRAME_D12_D13_PAIR) {
    registers.setFloatRegister(UNW_ARM_D12,
                               addressSpace.getDouble(savedRegisterLoc));
    savedRegisterLoc -= 8;
    registers.setFloatRegister(UNW_ARM_D13,
                               addressSpace.getDouble(savedRegisterLoc));
    savedRegisterLoc -= 8;
  }
  if (encoding & UNWIND_ARM_FRAME_D14_D15_PAIR) {
    registers.setFloatRegister(UNW_ARM_D14,
                               addressSpace.getDouble(savedRegisterLoc));
    savedRegisterLoc -= 8;
    registers.setFloatRegister(UNW_ARM_D15,
                               addressSpace.getDouble(savedRegisterLoc));
    savedRegisterLoc -= 8;
  }
  ...
  // set pc to be value in lr
  registers.setIP(registers.getRegister(UNW_ARM_LR));

  return UNW_STEP_SUCCESS;
}

template <typename A>
int CompactUnwinder_arm<A>::stepWithCompactEncodingFrame(
    compact_unwind_encoding_t encoding, uint64_t, A &addressSpace,
    Registers_arm &registers) {
#warning TODO(danakj): Is this right for ARM/NEON?

#warning TODO(danakj): There is no FP on ARM/NEON?
  uint64_t savedRegisterLoc = 0;//registers.getFP() - 8;
